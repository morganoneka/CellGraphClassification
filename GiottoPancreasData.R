...
for (group in groups){
  ...
  for (file in files){
    ...
    #TODO: get phenotype 
    data_in$CellType <- "Other"
    data_in[which(data_in$Epithelial == "pos"), "CellType"] = "Tumor"
    data_in[which(data_in$CD8 == "pos" & data_in$Tcell == "pos"), "CellType"] = "CTL"
    data_in[which(data_in$Treg == "pos"), "CellType"] = "T-reg"
    data_in[which(data_in$CD4 == "pos" & data_in$Tcell == "pos"), "CellType"] = "Helper T-Cell"
    data_in[which(data_in$APC == "pos"), "CellType"] = "APC"
...
# combine data into one file, separating the individuals by multiplying by some number
...
# ummmm
# cell_prox <- getCellProximityGeneScores(with_phenotype, spatial_network_name = "spatial_network", cluster_column="phenotype", in_two_directions = TRUE, do_diff_test=TRUE, minimum_unique_cells = 5)  
